<template>
  <div class="md calendar-month" :class="[dropState ? 'is-drop' : '']">
    <div class="calendar-caption">
      <div v-for="day in obj.week[0]" :key="'dayTag-' + day.language_day">
        {{ day.language_day }}
      </div>
    </div>
  ...
  methods: {
    ...
    changeData: function(obj) {
      ...
      if (this.moveType === "head") {
        if ($startTime.month === obj.month && $startTime.date === obj.date) {
          //同個事件拖到同一天
          return false;
        }
        ...
        let a = $startTime.year > obj.year,
          ...
          c =
            $startTime.year === obj.year &&
            $startTime.month === obj.month &&
            $startTime.date > obj.date;
    ...
    addClass: function(obj) {
      ...
      let $cover =
          new Date($endTime.year, $endTime.month - 1, $endTime.date) -
          new Date($startTime.year, $startTime.month - 1, $startTime.date),

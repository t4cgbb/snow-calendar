<template>
  <div class="week">
    ...
    <div class="events" @click.prevent="computedclickTime">
...
export default {
    ...
    events: function() {
      ...
      this.obj.forEach((date, date_i) => {
        ...
        if (
          ...
          this.event[date.year][date.month] &&
          this.event[date.year][date.month][date.date]
        ) {
          //先把一天一天的事件通通放入一個陣列
          let $target = this.$okaTool.copy(
            this.event[date.year][date.month][date.date]
          );

          $target.forEach((item, index) => {
            //過濾 cover 為 0 的跨天方法的第二天事件 && 沒有勾選顯示的物件
            let a =
                item.extend.cover === 0 && item.startTime.date !== date.date, //跨天方法的第二天事件
        ...
        $eventCopy.forEach(event => {
          //加入左側定位，寬度，預設為該天的位置以及 1 寬度，含有今天所有的事件
        ...
        if (date_i > 0) {
          ...
          $eventCopy.forEach((item, event_i) => {
            //有的話就開始調整位置，並加長第一次發生這個事件的寬度

            if (item) {
              let _sn = item.sn,
                _targetIndex = 0;

              let $searchResult = $prevEvent.find(search_item => {
                //靠 SN 確認前一天有沒有同一個事件
                if (search_item) {
              ...
              if ($searchResult !== undefined) {
                ...
                if ($searchResult.original === undefined) {
                  //如果沒有 original 這個屬性的話，代表這是第一次的複製物件，把第一次發生這個事件的物件加入參照
        ...
        $dayEvents.forEach((event, event_i) => {
          //排序完成，依序加入 top 定位
      ...
      $events.forEach(days => {
        ...
        days.forEach(item => {
          //事件
          if (item && item.original === undefined) {
            //只加入第一次發生的事件
            $results.push(item);
          }
        });
  ...
  methods: {
    ...
    computedclickTime: function(evt) {
      //用寬度算點擊的是哪一天
      let $self = this;
